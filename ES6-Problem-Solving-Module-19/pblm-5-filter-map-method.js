...
//using ES6
 //let getExpensiveItemsTotal=(cart, minPrice) =>{
  //   let filteredItem =cart.filter(product=> product.price >= minPrice)//filtered items
  //    //console.log(filteredItem)
  //    let modifiedItem = filteredItem.map(product => product.price * product.qty)//product price
  //    //console.log(modifiedItem)
  //    let total = modifiedItem.reduce((accumulator,product)=>accumulator+ product //sum
  ...
  let getExpensiveItemsTotal=(cart, minPrice) =>{
  let total = cart.filter(product => product.price>= minPrice)?.map(product => product.price * product?.qty)?.reduce((accumulator,product)=>accumulator+ product
  ,0,)
return total }
 let carts = [
      { name: "Pen", price: 20, qty: 3 },
   ...
   let minPrice = 500
    console.log(getExpensiveItemsTotal(carts,minPrice))
